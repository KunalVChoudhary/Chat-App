      ...
      <div className="flex flex-col">
        {
          userDummyData.map((user, index)=>(
            <div onClick={()=>{setUserSelected(user)}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${userSelected?._id === user._id && 'bg-[#282142]/50'}`}>
              <img src={user?.profilePic || assets.avatar_icon} alt="profile pic" className=" 
              ...
              <div className="flex flex-col leading-5">
                ...
                {
                  index < 2
                  ? <p className="absolute top-4 right-4 text-xs h-5 w-5 flex justify-center items-center rounded-full bg-voilet-500/50"> {index} </p>
                  : ''
                }
              </div>
            </div>
